...
function queryData(zip, radius, status)
{
    ...
    const settings = {
        "async": true,
        "crossDomain": true,
        "url": "https://cors-anywhere.herokuapp.com/https://api.yelp.com/v3/businesses/search?location=" + zip + ",radius=" + radius,  // + ",limit=50"
        "method": "GET",
    ...
    $.ajax(settings)
        ...
        .then(function (response) {
            ...
            if (status == "dine-in")
            {
                populateDataDineIn(1);
                $("#contentMain").hide();
                $("#restaurantContainer").show(); 
            }
            else
            {
                populateDataDelivery(1);
                $("#contentMain").hide();
                $("#restaurantContainer").show(); 
            }
        })
...
function populateDataDineIn(page)  //3
{
...
function populateDataDelivery(page)  //3
{
